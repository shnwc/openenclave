...
The OE SDK V0.10 release will have enclave-side verifier plugins for
verification of SGX local and ECDSA evidence,
...
for verification of OE reports for SGX generated by the legacy API function
`oe_get_report()` and raw SGX quotes generated by the Intel SGX SDK
DCAP library. These two evidence formats are identified by two UUIDs:
- `OE_FORMAT_UUID_SGX_REPORT`: for all OE reports generated
by the legacy API function `oe_get_report()`.
  - OE reports for both SGX local and remote attestation are supported.
- `OE_FORMAT_UUID_SGX_QUOTE`: for all raw SGX ECDSA quote generated
by the Intel SGX SDK DCAP library.
  - Note: currently, the DCAP library only generates ECDSA-p256 quotes.
  - In the future, if additional quote types are supported by the DCAP library,
  the same UUID would be used for verification of these new types of quotes.

There is no plan currently to implement verifier plugins for verification of
...
claims buffer. For evidence data in format `OE_FORMAT_UUID_SGX_REPORT`
or `OE_FORMAT_UUID_SGX_QUOTE`, since this evidence data embeds the
...
API function `oe_verify_evidence()` supports the values listed below in its
`format_id` parameter.
* `NULL`:
...
* `OE_FORMAT_UUID_SGX_ECDSA_P256`:
  * The input evidence is generated by `oe_get_evidence()` for format
  ...
  `OE_EVIDENCE_FLAGS_EMBED_FORMAT_ID` bit cleared in its `flags` parameter.
* `OE_FORMAT_UUID_SGX_REPORT`:
  * The input evidence is an OE report generated by the legacy API function
  `oe_get_report()`.
* `OE_FORMAT_UUID_SGX_QUOTE`:
  * The input evidence is a raw SGX ECDSA quote generated by the
  Intel SGX SDK DCAP library.
...
| `OE_FORMAT_UUID_SGX_ECDSA_P256` | `[ oe_attestation_header ] \|\| SGX_ECDSA_quote(hash) \|\| custom_claims` |
...
| `OE_FORMAT_UUID_SGX_REPORT` | `oe_report_header \|\| OE_report_body(custom_claims)` |
| `OE_FORMAT_UUID_SGX_QUOTE` | `SGX_quote(custom_claims)` |
